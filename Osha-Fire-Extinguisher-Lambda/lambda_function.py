...
def _link_session_to_inspection(session_id, inspection_id, inspection_type="fire-extinguisher"):
...
def compute_status(inspection):
    ...
    valid_items = [i for i in items if _numeric_item_id(i) not in (11, 12)]
    if not valid_items:
        return "in_progress"
...
def update_checklist_item(event):
    ...
    # ── Compute flat current_item_index ──
    flat_idx = 0
    current_idx = None
    for cat in inspection.get("categories", []):
        for item in cat.get("items", []):
...
def analyze_item_image(event, _is_async=False):
    ...
    if str(item_id).startswith("custom_"):
        ...
        _verdict_label, _verdict_display = resolve_verdict_fields(_ck, passed and not blocked, blocked)
    ...
    if str(item_id) == "2":
        ...
        def _item2_fail(finding, action, message, obj_det, cond, blocked=None):
            ...
            if _verdict_label:
                resp_body["blocked_verdict_label"] = _verdict_label
...
def resume_session(event):
    ...
    try:
        ...
        # ── Compute current_item_index for frontend ──────────────────
        flat_idx = 0
        current_idx = None
        for cat in inspection.get("categories", []):
            for item in cat.get("items", []):
        ...
        if current_idx is None:
            current_idx = flat_idx # Default to end if all answered
